...
Master.prototype.respawn = async function respawn(worker) {
  ...
  console.log('StickyMaster: Respawn process',index, this.options.autoRespawn);
...
Master.prototype.balance = function balance(socket) {
  ...
  const index = hash % (this.workers.length+2);
  //const index = Math.floor( Math.random()*(this.workers.length+1) );  //for testing
  //const index = 2; //for testing
  if (index === this.workers.length || this.options.disabled)
    return this.events.emit('sticky:balance', socket);
